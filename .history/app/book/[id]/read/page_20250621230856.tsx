...
export default function FullScreenReadPage({ params }: { params: { id: string } }) {
    ...
    const loadBookData = async () => {
      try {
        ...
        if (book) {
          ...
          const savedReadingMode = localStorage.getItem(`readingMode_${params.id}`)
          if (savedReadingMode) {
            setReadingMode(savedReadingMode as "light" | "dark" | "sepia")
          }
  ...
  const applyReadingModeToPdf = useCallback(() => {
    ...
    if (pdfPageElement) {
      ...
      if (canvas) {
        switch (readingMode) {
          case "dark":
            ...
            break
          case "sepia":
            canvas.style.filter = "sepia(1) saturate(0.8) brightness(1.1) contrast(1.1)"
      ...
      if (textLayer) {
        ...
        textSpans.forEach((span) => {
          switch (readingMode) {
            case "dark":
              ...
              break
            case "sepia":
              span.style.color = "rgba(92, 51, 23, 0.1)"
              break
            default:
        ...
        switch (readingMode) {
          case "dark":
            textLayerElement.style.filter = "invert(1) hue-rotate(180deg)"
            break
          case "sepia":
            textLayerElement.style.filter = "sepia(1) saturate(0.8) brightness(1.1)"
            break
          default:
  ...
  const handleReadingModeChange = (mode: "light" | "dark" | "sepia") => {
  ...
  // Get PDF container styles based on reading mode
  const getPdfContainerStyles = () => {
    switch (readingMode) {
      case "dark":
        return "bg-gray-800 p-4 rounded-lg"
      case "sepia":
        return "bg-amber-100 p-4 rounded-lg"
      default:
      ...
      <div className="flex flex-1 relative overflow-hidden">
        ...
        >
          ...
          <Button
            ...
            title="Sepia Mode (3)"
          >
            <div className="w-5 h-5 bg-amber-200 rounded-full border border-amber-400"></div>
          </Button>
        </div>
